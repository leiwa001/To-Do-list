...
class Front:
    def __init__(self, master, back):
        ...
        self.eingabefeld.bind("<Return>", lambda event: [back.add_task(self.eingabefeld.get()),
                                                          self.change_label_add_task(self.eingabefeld.get())])
        ...
        task_button = tk.Button(self.fenster, text="Bestätigen",
                                 command= lambda: [back.add_task(self.eingabefeld.get()),
                                                   self.change_label_add_task(self.eingabefeld.get())], bd=5)

        task_button.place(relx=0.41, rely=0.2, width=100, height=40)

        # Button, um Daten in Json zu speichern
        speicher_button = tk.Button(self.fenster, text="Speichern", command= lambda: [back.button_action_speichern(),
                                                                                  self.change_speicher_label()], bd=5)
        speicher_button.place(relx=0.2, rely=0.85, width=100, height=40)

        # Button, um Daten aus Json zu laden
        lade_button = tk.Button(self.fenster, text="Laden", command=lambda: [back.button_action_laden(self.aufgabenliste),
                                                                        self.change_lade_label()],  bd=5)
        ...
        loesch_button = tk.Button(self.fenster, text="Löschen", command= lambda: [back.button_action_loeschen(self.aufgabenliste),
                                                                             self.change_loesch_label()], bd=5)
    ...
    def create_new_window(self, back):
        ...
        speicher_button = tk.Button(self.new_window, text="Speichern",
                                    command= lambda: [back.bearbeitung_speichern(
                                        self.textfeld.get("1.0", tk.END), self.sel_dict),self.new_window.destroy()], bd=5)
        ...
        self.cal_button = tk.Button(self.new_window, text="Auswählen", 
                                    command= lambda: [back.get_datum(self.sel_dict,self.cal.get_date()),
                                                      self.cal_label.config(
                                                          text="Fälligkeitstermin: " + self.cal.get_date())], bd=5)
        ...
        check = tk.Checkbutton(self.new_window, text="", variable=self.var1, onvalue=1, offvalue=0,
